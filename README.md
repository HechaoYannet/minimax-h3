...
7. ~~**参考视频/音频的端到端仍未跑过**~~ → **已跑通，并且修掉了一个必崩的 bug**，见 §12。四条路径（静音视频 / 视频+音轨 / 纯音频 / 图+音频）现在都能出片。
...
> 数据来源：`scripts/h3_sweep.py`（speed 15 点 / capability 11 点，每点都是一次完整流水线），
> 原始记录在 `cache/sweep/*.json`，复现：`python3 scripts/h3_sweep_report.py`。
> 下面每个数字都是跑出来的，不是外推。
...
| | |
|---|---|
| **最快** | `blitz` 预设 640×384×22f / 8 步 / 参考图短边 256 → **约 46 s 出片**（0.92 s 视频） |
| **性价比拐点** | 39 帧比 22 帧多 0.7 s 视频只多花约 10 s；再往上每多 1 s 视频要 ~40 s |
| **交付** | `standard` 832×480×124f / 30 步 → **17 分钟**（5.17 s 视频，峰值 4.55 GiB） |
| **最长可用** | 640×384×**243 帧**（10.12 s 视频）跑通；832×480×243f 与 640×384×345f 超显存 |
| **硬天花板** | **官方原生 1344×768 在 8 GiB 卡上跑不了**（干净状态、单独跑也 OOM） |

### 11.1 速度谱（`cache/sweep/speed.json`）

| 配置 | 帧 | 步 | 参考短边 | s/step | 去噪 | 解码 | 流水线 | 出片 | 实时倍率 | 峰值 VRAM |
|---|---|---|---|---|---|---|---|---|---|---|
| `blitz` | 22 | 8 | 256 | **5.58** | 49.5 s | 2.7 s | **52.2 s** | 0.92 s | 57× | 3.30 |
| `fast` | 39 | 8 | 256 | 8.22 | 71.0 s | 4.5 s | 75.5 s | 1.62 s | 47× | 3.29 |
| 640×384 | 22 | 4 | 768 | 7.90 | 48.8 s | 3.9 s | 52.7 s | 0.92 s | 57× | 3.23 |
| 640×384 | 22 | 20 | 768 | 8.01 | 177.0 s | 3.9 s | 180.9 s | 0.92 s | 197× | 3.23 |
| `draft` | 73 | 8 | 768 | 13.42 | 111.7 s | 8.2 s | 119.9 s | 3.04 s | 39× | 2.87 |
| `draft` | 73 | 20 | 768 | 13.59 | 277.7 s | 8.0 s | 285.8 s | 3.04 s | 94× | 3.05 |
| 384×224 | 39 | 8 | 768 | 7.12 | 60.5 s | 2.0 s | 62.5 s | 1.62 s | 39× | 3.30 |
| 448×256 | 39 | 8 | 768 | 7.62 | 72.0 s | 3.0 s | 75.0 s | 1.62 s | 46× | 3.36 |
| 512×288 | 39 | 8 | 768 | 8.10 | 69.5 s | 5.5 s | 75.0 s | 1.62 s | 46× | 3.25 |

**三条规律**，直接决定该怎么选预设：

1. **短片段有一个硬底 ~5.6 s/step。** 22 帧不管跑 4 步还是 20 步，单步都是 7.9~8.0 s —— 序列长短在这里几乎不起作用，因为整个前向才 ~1.5 TFLOP，单步成本被那个固定底吃满了。
2. **参考图短边是最陡的性能杠杆。** 同样 39 帧 8 步：短边 256 → 8.22 s/step，1536 → **17.61 s/step**（2.1×）。
3. **总时长 ≈ 固定 ~15 s + 单步 × 步数。** 固定部分 = 参考解码（约 6 s）+ 文本编码（首跑 12 s / 缓存命中 0 s）+ VAE 解码（2~8 s）+ 模型换入换出。短片段里固定部分占比极高。

### 11.2 能力谱（`cache/sweep/capability.json`）

| 配置 | 帧 | 步 | s/step | 流水线 | 出片 | 峰值 VRAM |
|---|---|---|---|---|---|---|
| 640×384 | 124 | 20 | 20.09 | 422 s | 5.17 s | 3.25 |
| 832×480 | 73 | 20 | 21.18 | 443 s | 3.04 s | 3.43 |
| 960×544 | 73 | 20 | 26.44 | 550 s | 3.04 s | 3.88 |
| 1024×576 | 73 | 20 | 28.62 | 594 s | 3.04 s | 4.16 |
| **832×480** | **124** | **30** | **33.36** | **1028 s（17 min）** | **5.17 s** | **4.55** |
| 640×384 | **243** | 20 | 37.07 | 987 s（16 min） | **10.12 s** | 4.93 |
| **1024×576** | **124** | **30** | **48.99** | **1503 s（25 min）** | **5.17 s** | **5.94** |
| 640×384 | 345 | 20 | — | **OOM** | — | — |
| 832×480 | 243 | 30 | — | **OOM** | — | — |
| **1344×768（官方原生）** | 124 | 30 | — | **OOM** | — | — |

**显存墙在哪**：失败的三个点都在「长序列 **且** 大画布」的区域。1024×576×124（5.94 GiB 峰值）是跑通的最高点；再往上激活就装不下了。
640×384 能把时长推到 243 帧（10.1 s）正因为它把画布压到了最小 —— **时长和分辨率只能二选一**。

**要更正 §0/§5 的旧说法**：官方原生档不是「慢到 70 分钟」，而是**本机根本跑不了**。

### 11.3 三个反直觉的实测结果

**（1）`vram_limit` 在短序列上依然几乎没用。** 22 帧 5 步的 A/B：

| vram_limit | s/step（稳态） | 峰值 VRAM |
|---|---|---|
| 4.58（默认） | 5.90 | 3.30 GiB |
| 5.60 | 5.69 | 4.42 GiB |
| 6.30 | 5.56 | 5.17 GiB |

**多花 57% 显存换 6% 速度**。默认 4.58 保持不动。（7232 token 上的同向实验：常驻 25 → 70 层，13.17 → 13.34 s/step，同样是零收益。）

**（2）一次 OOM 会污染同一台机器上后续所有配置。**

这是本轮最坑的发现。`cap_1344x768_124_s30` 真的超显存之后，**紧接着**的 `std_832x480_124_s30`（一个已知能跑的配置）也立刻报同样的
`CUDA driver error: device not ready / Failed to create GPU mapping`。单独重跑 `cap_1024x576_124_s30` 则完全正常（48.99 s/step，跑满 25 分钟）。

所以：**批量跑配置时，任何一次 OOM 都会污染后面所有结果。** 可能超限的配置必须单独验，或者 OOM 后 `wsl --shutdown`。
`cache/sweep/capability.json` 里那 3 个 FAILED 就是这么来的，已逐个单独复验（构成 §11.2 的结论）。

**（3）跑长片段会顶到主机内存，而不是显存。** 看门狗记录（`cache/sweep/watch.log`）：243 帧跑到一半时 `ram free` 只剩 **144 MiB**，
而显存还有 3 GiB。DiT 常驻 CPU 要 9.5 GiB，冷启动的文本编码器再 mmap 14.27 GiB —— 两者相加就顶到 22.9 GiB 的 WSL 配额。
曾经因此把整个 WSL 发行版打死过一次（连 traceback 都没留下）。

对策：长片段一律用 `--dit-onload disk`（DiT 走 mmap，不落地到 RAM），实测同尺寸下**比常驻 CPU 还快**（832×480×73：21.18 vs 各向同性；832×480×124：33.36 s/step）。

### 11.4 两个推荐档位

```bash
# 最大速度：约 46 s，0.92 s 视频。试种子 / 试 prompt / 试构图用这个。
./run_h3.sh gen --preset blitz \
  --prompt-file workspace/pink-harem-ref2va/prompt.txt \
  --ref-image workspace/pink-harem-ref2va/materials/ref-pink-harem.jpg \
  --lora ~/source/minimax-h3/models/AfterMidnight_ref2va_h3_softer_rank64_v1.safetensors \
  --out workspace/pink-harem-ref2va/outputs/blitz.mp4

# 最大能力：约 35 分钟，10.12 s 视频。本机验证过的最长可用档。
./run_h3.sh gen --preset max-long ... --out .../max-long.mp4
```

`fast`（39 帧 / 10 步 / 参考图短边 256，约 87 s，1.62 s 视频）是**性价比拐点**：
视频长度是 blitz 的 1.76 倍，时间只多 67%；再往上一档（73 帧）时间翻倍而视频只多 88%。

### 11.5 本轮新增的工具

| 工具 | 用途 |
|---|---|
| `scripts/h3_sweep.py` | 跑预设矩阵，每配置输出一行 `BENCH {json}`；带内存看门狗，崩溃前留下 RAM/VRAM 曲线 |
| `scripts/h3_sweep_report.py` | 把 `cache/sweep/*.json` 渲染成上面的表 |
| `scripts/h3_assess.py` | 成品客观体检：帧差（冻结/崩塌/运动量）、音频 RMS/削顶/NaN |
| `scripts/h3_quality_sweep.py` | 步数与参考图短边的画质对照，产出真实 mp4 并自动体检 |
| `--bench --label X` | 只跑一遍流水线并打印机器可读记录，不写视频 |
| `--activation-reserve G` | 按 `vram_limit = 启动可用 − G − LoRA` 反推 |
| 形状对齐告警 | 非法帧数（`%17!=5`）会被明确记录并吸附，不再静默换一个片段长度 |

---

## 12. 参考视频/音频全链路：修掉了一个从未被跑到的崩溃

§8.7 一直挂着一句「参考视频/音频的端到端还没跑过」。本轮跑了，**四条里三条直接崩**。
...
|---|---|---|
| `--ref-video`（静音视频） | ✅ 能跑 | ✅ |
| `--ref-audio` | ❌ `RuntimeError: No available kernel. Aborting execution.` | ✅ |
| `--ref-video-audio`（官方 tav2va 用法） | ❌ 同上 | ✅ |
| `--ref-image` + `--ref-audio` | ❌ 同上 | ✅ |

崩点固定在 `MiniMaxH3Unit_ReferenceEncoder._encode_audio_ref` → `MiniMaxH3AudioVAE.encode_audio`
→ `CausalAttention` → `torch_sdpa`。**只要参考里带音频就必崩**，纯图/纯静音视频永远碰不到。

### 12.2 根因：整条流水线被套在一个 `sdpa_kernel(CUDNN_ATTENTION)` 里

`h3_generate.py` 原来是这么写的：

```python
ctx = sdpa_kernel(getattr(SDPBackend, backend))   # backend 默认 CUDNN_ATTENTION
ctx.__enter__()
video, audio = pipe(...)                          # 整条流水线都在这个上下文里
```

这个上下文是**全局**的：它把**每一次** SDPA 调用都限制成只用 cuDNN。而音频 VAE 的因果自注意力
（`q_pattern="b s (n d)"` + `is_causal=True`）**cuDNN 没有对应 kernel**，于是直接 abort。

在真实流水线里逐个后端实测（`workspace/_instr7.py` 的做法）：

| 音频 VAE 注意力用的后端 | 结果 |
|---|---|
| 默认（不强制） | ❌ `No available kernel` |
| `CUDNN_ATTENTION` | ❌ `No available kernel` |
| `MATH` | ❌ `CUDA driver error: device not ready`（要物化整个注意力矩阵，这些长度下直接 OOM） |
| `FLASH_ATTENTION` | ✅ |

注意第一行：**即使完全不强制，自动选择也会失败**。（`torch.backends.cuda.sdp_kernel` 已废弃、
`SDPBackend` 是 pybind11 枚举不能重排成员顺序，两条弯路都试过。）

### 12.3 修法：两条注意力路径分开配后端

一条流水线里其实有**两条**注意力路径：

| 路径 | 走哪个函数 | 用什么后端 |
|---|---|---|
| DiT | `MiniMaxH3DiT._sdpa_varlen_attention` | `--sdpa-backend`（默认 cudnn，实测最快） |
| 其它（音频 VAE 等） | `diffsynth.core.attention.attention_forward` → `torch_sdpa` | 固定 `FLASH_ATTENTION` |

`prefer_sdpa_backend()` 现在只包住通用路径的 `torch_sdpa`（用可重入的 `sdpa_kernel` 逐次进入），
DiT 那条路径不再被全局上下文污染。

**回归验证**：`--preset draft --steps 5` 单步 14.18 s（基线 13.6~14.5 s），DiT 仍然走 cudnn，没有代价。

### 12.4 修好之后四条路都跑通（`outputs/refmodes/`）

| 产物 | 音频 RMS | 帧差 | 说明 |
|---|---|---|---|
| `ref-video.mp4` | 0.00027 | 0.0158 | 静音参考（本来就不该有强音频） |
| `ref-video-audio.mp4` | 0.02343 | 0.0154 | 官方 tav2va：视频+自带音轨 |
| `ref-audio.mp4` | **0.06342** | 0.0101 | 纯音频参考，输出音频最强 |
| `mixed.mp4` | 0.05822 | 0.0167 | 图 + 音频混合顺序 |

音频条件确实起作用了：**带音频参考的输出 RMS 比静音参考高 87~235 倍**。

测试素材由 `workspace/mk-reference-media.sh` 用 ffmpeg 从同一张参考图合成（Ken-Burns 推镜 + 正弦音），
所以整条测试可以从已提交的素材复现，不依赖任何二进制文件。

### 12.5 顺带发现的宿主内存墙

跑到 243 帧时看门狗记录到 `ram free` 只剩 **144 MiB**（`cache/sweep/watch.log`），而显存还剩 3 GiB。
DiT 常驻 CPU 占 9.5 GiB，冷启动文本编码器再 mmap 14.27 GiB，两者相加就顶到 22.9 GiB 的 WSL 配额 ——
本轮因此**把整个 WSL 发行版打死过一次**（连 traceback 都没留下，只能 `wsl --shutdown` 重启）。
